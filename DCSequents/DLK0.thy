...
axioms
 ...
 axioms
  ...
  allL:  "$H, P(t), $G, WALL x P(x) |- $E ==> $H, WALL x P(x), $G |- $E"

  exR:   "$H |- $E, P(t), $F, WEX x P(x) ==> $H |- $E, WEX  x P(x), $F"
...
lemma Less_fact[simp] : "close (e [<=] f) == e [<=] f"
apply (rule iff_reflection)
apply (simp add:equal_less_def)
apply fast
done 

lemma Greater_fact[simp] : "close (e [>=] f) == e [>=] f"
apply (rule iff_reflection)
apply (simp add:equal_greater_def)
apply fast
done 

(*New added axioms for proving arithmetic formulas of our syntax: to transform them to HOL formulas*)
(*Arith*)
...
Trans4: "(formT (f1::fform) & formT (f2::fform) & formT (f3::fform) & formT (f4::fform)==> formT (f::fform)) ==> f1, f2, f3, f4 |- f" and
Trans22: "((formT (f::fform)& formT (g::fform)) ==> (formT (f1::fform) | formT (f2::fform) | formT (f3::fform))) ==> f, g  |- f1, f2, f3"  and
Transr4: " (formT (f1::fform) | formT (f2::fform) | formT (f3::fform) | formT (f4::fform)) ==>  |- f1, f2, f3, f4" and
Transr3: " (formT (f1::fform) | formT (f2::fform) | formT (f3::fform)) ==>  |- f1, f2, f3" 
